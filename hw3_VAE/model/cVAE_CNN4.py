...
class cVAE_Encoder(nn.Module):
    def __init__(self, input_image, latent_z, class_num, device):
        ...
        #第2種捲積方式
        self.conv1_ = nn.Conv2d(in_channels=4,
                               out_channels=16,
                               kernel_size=5,
                               stride=2)
        self.conv2_ = nn.Conv2d(in_channels=16,
                               out_channels=32,
                               kernel_size=5,
                               stride=2)
        ...
        self.liner1 = nn.Linear(256*6*6, 512)
...
class cVAE_Decoder(nn.Module):
    def __init__(self, input_image, latent_z, class_num, device):
        ...
        self.liner2 = nn.Linear(512, 256*6*6)
        
        self.conv1T = nn.ConvTranspose2d(in_channels=256,
                                         out_channels=128,
                                         kernel_size=kernel_size,
                                         stride=stride,
                                         padding=kernel_size//stride+1)
        self.conv2T = nn.ConvTranspose2d(in_channels=128,
                                         out_channels=64,
                                         kernel_size=kernel_size,
                                         stride=stride,
                                         padding=kernel_size//stride+1)
        self.conv3T = nn.ConvTranspose2d(in_channels=64,
                                         out_channels=3,
                                         kernel_size=kernel_size,
                                         stride=stride,
                                         padding=kernel_size//stride+1)
...
if __name__ == '__main__':
    ...
    latent_z = 64
    class_num = 10
    model = cVAE_CNN4(input_image = input_image,
                    latent_z = latent_z,
                    class_num = class_num,
